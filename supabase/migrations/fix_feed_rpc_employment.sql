-- ==============================================================================
-- FIX: Safe get_user_feed RPC with Correct Column Mappings & Performance
-- ==============================================================================
-- Resolves error 42703: "column u.employment_status does not exist"
-- In public.users, employment is stored in column "employment".
-- ==============================================================================

-- 1. Drop existing overloaded signatures to avoid PGRST203 ambiguity
...
-- 2. Create canonical get_user_feed RPC
...
BEGIN
  ...
  FROM public.users u
  WHERE lower(trim(u.email)) = v_email_lower
  LIMIT 1;
  ...
  WITH candidate_ads AS (
    ...
    WHERE a.completed_at IS NULL
      ...
      AND LOWER(trim(a.user_email)) != v_email_lower
      AND (
        -- Mutual target direct bypass
        (a.mutual_targets IS NOT NULL AND v_email_lower = ANY(
          ARRAY(SELECT lower(trim(t)) FROM unnest(a.mutual_targets) t)
        ))
        OR (
      ...
      AND NOT EXISTS (
        ...
        WHERE imp.ad_id = a.id
          AND lower(trim(imp.user_email)) = v_email_lower
